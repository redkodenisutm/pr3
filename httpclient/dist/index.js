...
const httpClient = async () => {
    let res = await instance.get("/candidates");
    console.log({ candidates: res.data });
    res = await instance.get("/offers");
    ...
    let candidatesStudentsAtUtm = res.data.filter((candidate) => regex.test(candidate.email));
    console.log({ candidates: candidatesStudentsAtUtm });
    res = await instance.options("/options");
    console.log("Server options:", res.headers);
    const jsons = ["offers", "companies", "candidates", 'bin'];
    res = await instance.options("/options");
    for (const json of jsons) {
        try {
            ...
            console.log("probably file not found");
        }
    }
};
httpClient();
